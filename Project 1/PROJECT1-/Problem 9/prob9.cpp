...
int main(){
    ...
    for(int i = 1; i < 6; i++){                     //Iterate N from 10 to 10^7 
        ...
        string filename = "prob9" + to_string(N) + ".txt";  
        ...
        for (int i = Nnew-2; i >= 0 ; i--){         // Loop through N-2 indexes downwards starting from end index
            vt(i) = (gt(i)+vt(i+1))/bt(i);          // v-stjerne vector def
        }
    
        // Writing them into the file
        for (int i=0; i<x.size(); i++){
            ofile << setw(19) << setprecision(10) << scientific << x(i);             // x-values
            ofile << setw(19) << setprecision(10) << scientific << vt(i);            // approx values
            ofile << setw(19) << setprecision(10) << scientific << ux(i) << endl;    // exact values
        }
