...
PenningTrap::PenningTrap(double B0_in, double V0_in, double d_in, bool l_in){
  ...
  vec PenningTrap::total_force(int i){
    if (m_l == false){  // Without particle interaction
  ...
  void PenningTrap::evolve_Euler_Cromer(double dt, int l){
    m_all_p[0].m_v += dt*(total_force(0)/m_all_p[0].m_m);
