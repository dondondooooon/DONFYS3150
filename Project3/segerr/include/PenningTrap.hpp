...
public: // Public 
  ...
  int m_psize;  // Number of Particles
  bool m_colm;  // Coloumb Interaction
  double m_f;   // Constant Amplitude
  double m_wv;  // Angular Frequency
  vec m_t;      // Time Vector
  ...
  // Constructor
  PenningTrap(double B0_in, double V0_in, double d_in, bool colm_in, double f_in, double wv_in, double n);
  ...
  vec external_E_field(vec r, double t);  

  // External magnetic field at point r=(x,y,z)
  vec external_B_field(vec v, vec r);
...
// The total force on particle_i from the external fields
  ...
  void evolve_Euler_Cromer(double dt, int i, int j);

  // Full Evolution of the system using Runge-Kutta 4th order
  void full_evolution(double dt, double n, int psiz);
